...
const directoryCommentSchema = z.object({
...
/**
 * GET /api/directory/[id]/comments - List comments for a directory listing
 */
...
/**
 * POST /api/directory/[id]/comments - Create a comment on a directory listing
 */
export async function POST(
  ...
  try {
    ...
    const parsed = directoryCommentSchema.safeParse(body);
    if (!parsed.success) {
      return NextResponse.json(
    ...
    // Notify listing owner (if not self-commenting)
    if ((listing as any).user_id !== auth.user.id) {
