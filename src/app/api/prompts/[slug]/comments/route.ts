...
const promptCommentSchema = z.object({
...
/**
 * GET /api/prompts/[slug]/comments - List comments for a prompt listing
 */
...
/**
 * POST /api/prompts/[slug]/comments - Create a comment on a prompt listing
 */
export async function POST(
  ...
  try {
    ...
    const parsed = promptCommentSchema.safeParse(body);
    if (!parsed.success) {
      return NextResponse.json(
    ...
    if ((listing as any).status !== "active") {
      return NextResponse.json({ error: "Prompt is not available" }, { status: 400 });
    }
    ...
    if ((listing as any).seller_id !== auth.user.id) {
      await (admin.from("notifications") as any).insert({
        ...
        type: "prompt_purchased", // reuse notification type
