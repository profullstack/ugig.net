...
const skillCommentSchema = z.object({
...
/**
 * GET /api/skills/[slug]/comments - List comments for a skill listing
 */
...
/**
 * POST /api/skills/[slug]/comments - Create a comment on a skill listing
 */
export async function POST(
  ...
  try {
    ...
    const parsed = skillCommentSchema.safeParse(body);
    if (!parsed.success) {
      return NextResponse.json(
    ...
    if ((listing as any).status !== "active") {
      return NextResponse.json({ error: "Skill is not available" }, { status: 400 });
    }
