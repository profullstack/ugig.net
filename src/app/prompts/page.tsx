...
async function PromptList({ searchParams }: { searchParams: PromptsPageProps["searchParams"] }) {
  ...
  query = excludeBlocked(query, "seller_id", await getSessionBlockedIds(supabase));
      ...
      <div className="text-center py-12 bg-muted/30 rounded-lg">
        ...
        <p className="text-muted-foreground mb-2">
          {queryParams.search || queryParams.category || queryParams.tag
            ...
            : "No prompts listed yet. Be the first to publish one!"}
        </p>
        <div className="flex items-center justify-center gap-3 mt-4">
          ...
          <Link href="/dashboard/prompts/new">
            <Button size="sm">Publish a Prompt</Button>
          </Link>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      <p className="text-sm text-muted-foreground">
        Showing {listings.length} of {count} prompts
      </p>
          ...
          >
            ...
            {listing.model_compatibility && listing.model_compatibility.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mb-3">
                {listing.model_compatibility.slice(0, 3).map((model: string) => (
                  <span
                    key={model}
                    className="text-xs px-2 py-0.5 bg-blue-500/10 text-blue-600 rounded-full"
                  >
                    {model}
                  </span>
                ))}
                {listing.model_compatibility.length > 3 && (
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex flex-wrap gap-3 mb-6">
            ...
            <div className="flex flex-wrap gap-1.5">
              <Link href={`/prompts?${queryParams.search ? `search=${queryParams.search}&` : ""}${queryParams.tag ? `tag=${queryParams.tag}&` : ""}${queryParams.sort ? `sort=${queryParams.sort}` : ""}`}>
                <Badge
              ...
              {PROMPT_CATEGORIES.map((cat) => (
                <Link
                  key={cat}
                  href={`/prompts?category=${cat}${queryParams.search ? `&search=${queryParams.search}` : ""}${queryParams.tag ? `&tag=${queryParams.tag}` : ""}${queryParams.sort ? `&sort=${queryParams.sort}` : ""}`}
                >
                  <Badge
            ...
            <div className="ml-auto flex items-center gap-2 text-sm">
              ...
              {[
                ...
                <Link
                  key={value}
                  href={`/prompts?sort=${value}${queryParams.search ? `&search=${queryParams.search}` : ""}${queryParams.category ? `&category=${queryParams.category}` : ""}${queryParams.tag ? `&tag=${queryParams.tag}` : ""}`}
          ...
          {queryParams.tag && (
            <div className="flex items-center gap-2 mb-6">
              ...
              <Badge variant="secondary" className="gap-1">
                {queryParams.tag}
                <Link href={`/prompts?${new URLSearchParams({
                  ...(queryParams.search ? { search: queryParams.search } : {}),
                  ...(queryParams.category ? { category: queryParams.category } : {}),
                  ...(queryParams.sort ? { sort: queryParams.sort } : {}),
