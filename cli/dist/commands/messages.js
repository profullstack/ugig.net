...
export function registerMessagesCommands(program) {
    ...
    msgs
        ...
        .option("--limit <n>", "Number of messages", (v) => Number(v), 50)
    ...
    msgs
        ...
        try {
            const client = createClient(opts);
            const result = await client.post(`/api/gigs/${gigId}/messages`, { message: options.message });
            spinner?.succeed("Message sent");
            if (opts.json) {
                console.log(JSON.stringify(result, null, 2));
            }
            else {
                printSuccess(`Message sent. Conversation: ${result.conversation_id}`, opts);
            }
        }
        catch (err) {
            spinner?.fail("Failed to send message");
