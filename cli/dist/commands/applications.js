...
import { printTable, printSuccess, truncateId, truncate, colorizeStatus, relativeDate, } from "../output.js";
export function registerApplicationsCommands(program) {
    ...
    apps
        ...
        try {
            ...
            const result = await client.get("/api/applications/my");
            spinner?.stop();
            printTable([
                { header: "ID", key: "id", width: 12, transform: truncateId },
                { header: "Gig", key: "gig", width: 30, transform: (v) => truncate(28)(v?.title) },
    ...
    apps
        ...
        try {
            const client = createClient(opts);
            const result = await client.get(`/api/gigs/${gigId}/applications`);
            spinner?.stop();
            printTable([
                { header: "ID", key: "id", width: 12, transform: truncateId },
    ...
    apps
        ...
        try {
            ...
            printSuccess(`Application status changed to ${options.status}.`, opts);
        }
        catch (err) {
    ...
    apps
        ...
        try {
            ...
            const result = await client.put("/api/applications/bulk-status", {
                application_ids: ids,
                status: options.status,
            });
            spinner?.succeed(`Updated ${result.updated} applications`);
            printSuccess(`${result.updated} applications updated to ${options.status}.`, opts);
        }
        catch (err) {
...
export function registerApplyShortcut(program) {
    program
        ...
        try {
            ...
            if (options.rate !== undefined)
                body.proposed_rate = options.rate;
            if (options.timeline)
                body.proposed_timeline = options.timeline;
            if (options.portfolio)
                body.portfolio_items = parseList(options.portfolio);
            if (options.aiTools)
                body.ai_tools_to_use = parseList(options.aiTools);
            const result = await client.post(`/api/gigs/${gigId}/applications`, body);
            spinner?.succeed("Application submitted");
            if (opts.json) {
                console.log(JSON.stringify(result, null, 2));
            }
            else {
                printSuccess(`Application submitted: ${result.application.id}`, opts);
            }
        }
        catch (err) {
