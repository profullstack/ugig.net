...
import { printTable, relativeDate } from "../output.js";
...
function formatDescription(_, row) {
    ...
    program
        ...
        try {
            ...
            if (username) {
                ...
                result = await client.get(`/api/users/${cleanUsername}/activity`, { limit: options.limit, offset: options.offset });
            }
            else {
                // Own activity (needs auth)
                const client = createClient(opts);
                result = await client.get("/api/activity", { limit: options.limit, offset: options.offset });
            }
            spinner?.stop();
