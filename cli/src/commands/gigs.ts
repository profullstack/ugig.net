...
export function registerGigsCommands(program: Command): void {
  ...
  gigs
    ...
    .action(async (options) => {
      ...
      try {
        ...
        const result = await client.get<{ gigs: Record<string, unknown>[]; pagination: Record<string, number> }>("/api/gigs", params);
        spinner?.stop();
        printTable(
          [
          ...
          { page: result.pagination.page, total: result.pagination.total, totalPages: result.pagination.totalPages }
        );
      } catch (err) {
  ...
  gigs
    ...
    .action(async (id: string) => {
      ...
      try {
        const client = createUnauthClient(opts);
        const result = await client.get<{ gig: Record<string, unknown> }>(`/api/gigs/${id}`);
        spinner?.stop();
        printDetail(
          [
            { label: "ID", key: "id" },
  ...
  gigs
    ...
    .action(async (options) => {
      ...
      try {
        ...
        const result = await client.post<{ gig: Record<string, unknown> }>("/api/gigs", body);
        spinner?.succeed("Gig created");
        if (opts.json) {
          console.log(JSON.stringify(result, null, 2));
        } else {
          printSuccess(`Gig created: ${result.gig.id}`, opts as OutputOptions);
        }
      } catch (err) {
  ...
  gigs
    ...
    .action(async (id: string, options) => {
      ...
      try {
        ...
        if (options.skills !== undefined) body.skills_required = parseList(options.skills);
        if (options.aiTools !== undefined) body.ai_tools_preferred = parseList(options.aiTools);
        ...
        printSuccess("Gig updated successfully.", opts as OutputOptions);
      } catch (err) {
  ...
  gigs
    ...
    .action(async (id: string) => {
      ...
      try {
        ...
        printSuccess("Gig deleted successfully.", opts as OutputOptions);
      } catch (err) {
  ...
  gigs
    ...
    .action(async () => {
      ...
      try {
        const client = createClient(opts);
        const result = await client.get<{ gigs: Record<string, unknown>[] }>("/api/gigs/my");
        spinner?.stop();
        printTable(
          [
            { header: "ID", key: "id", width: 12, transform: truncateId },
  ...
  gigs
    ...
    .action(async (id: string, options) => {
      ...
      try {
        ...
        printSuccess(`Gig status changed to ${options.status}.`, opts as OutputOptions);
      } catch (err) {
