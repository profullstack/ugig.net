...
import { printTable, printSuccess, type OutputOptions, truncateId, formatDate, relativeDate } from "../output.js";
  ...
  keys
    ...
    .action(async () => {
      ...
      try {
        ...
        const result = await client.get<{ keys: Record<string, unknown>[] }>("/api/api-keys");
        spinner?.stop();
        printTable(
          [
  ...
  keys
    ...
    .action(async (options) => {
      ...
      try {
        ...
        const result = await client.post<Record<string, unknown>>("/api/api-keys", body);
  ...
  keys
    ...
    .action(async (id: string) => {
      ...
      try {
        ...
        printSuccess("API key revoked.", opts as OutputOptions);
      } catch (err) {
        spinner?.fail("Failed");
        handleError(err, opts as OutputOptions);
      }
